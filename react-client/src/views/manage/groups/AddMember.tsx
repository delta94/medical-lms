...
import {emptyUser, User, UserApi} from "../../../api/v1/UserApi";
...
export default function AddMember(props: IAddMemberProps) {
    ...
    async function getSuggestion(value): Promise<User[]> {
        query.searchTerm = value.trim();

        let users = await UserApi.find(props.clientId, query);
    ...
    function submit(): void {
        GroupApi.addMember(props.clientId, props.groupId, selected.id)
            .then(_ => {
                props.confirm();
            });
    }
    ...
    return (
        <Modal show={true} centered={true} onHide={props.hide}>
            <ModalHeader closeButton>
                <ModalTitle>{t("add-member")}</ModalTitle>
            </ModalHeader>
            <ModalBody>
